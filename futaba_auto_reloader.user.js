// ==UserScript==
// @name           futaba auto reloader m
// @namespace      https://github.com/messala36/futaba_auto_reloader_m
...
(function ($) {
    ...
    function deleteFooterSpace() {
        // class=dmps のdivタグの前のdivタグを削除
        $(".dmps").prev().remove();
        // id=ufmのdivタグの次のdivタグを削除
        $("#ufm").next().remove();
    }

    function popupReplyForm() {
        const formId = 'fm';
        ...
        toggleBtn.innerText = '▼'; // 初期は閉じている状態
        ...
        // フォームを固定位置に表示
        // まずフォームを document.body 直下に移動
        // ただし、もともとのDOMからは削除せず複製もなし
        // 既に表示済みなら何もしない
        // スタイル調整で画面上に固定表示
        ...
        // 直下の子要素の中からクラス 'ftb2' の table を消す(フォームのスリム化)
        form.querySelectorAll('table.ftb2').forEach((table) => {
            table.remove(); // 見つけ次第削除(基本1つのはず)
        });
    ...
    function setWheelEvent() {
        ...
        function wheelAndTouchmoveHandler(deltaY) {
